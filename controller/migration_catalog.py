...
_HORIZON_TABLES: Final[tuple[str, ...]] = (
    "horizon_projects",
    "horizon_project_nodes",
    "horizon_prerequisite_decisions",
    "horizon_correlation_qa",
)
...
_ROUTINES_014: Final[tuple[str, ...]] = _ROUTINES_013 + (
    "longspan_ingest_project_program(TEXT, TEXT, TEXT, TEXT, JSONB)",
    "longspan_get_project_node_ledger(TEXT, TEXT, TEXT)",
)
_ROUTINES_015: Final[tuple[str, ...]] = _ROUTINES_014 + (
    "longspan_create_subworkflow_handoff(TEXT, TEXT, TEXT, BIGINT, INTEGER, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, TEXT, TEXT, INTEGER)",
    "longspan_complete_subworkflow_handoff(TEXT, TEXT, TEXT, TEXT, BIGINT, INTEGER, JSONB, TEXT)",
    "longspan_expire_subworkflow_handoff(TEXT, TEXT, INTEGER, TEXT)",
)
_ROUTINES_016: Final[tuple[str, ...]] = _ROUTINES_015 + (
    "longspan_detect_missing_prerequisites(TEXT, TEXT, TEXT, JSONB)",
    "longspan_record_prerequisite_decision(TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT)",
    "longspan_record_correlation_qa(TEXT, TEXT, TEXT, JSONB, TEXT, JSONB, TEXT)",
    "longspan_get_correlation_answer(TEXT, TEXT)",
)
...
EXPECTED_MIGRATION_CATALOG_DIGEST: Final[str] = (
    "cdbf302e184ca13f7d149bba77a15ed30a58b63c15d140f3d7dee9db72a8b617"
)
...
def _expected_sql_allowlist_arrays(
    ...
    if revision in {
        ...
        if field == "public_routines":
            if revision in {"014_horizon_project_ledger", "018_horizon_project_ledger_live"}:
                return (_ROUTINES_014,)
            if revision in {"015_subworkflow_handoff", "019_subworkflow_handoff_live"}:
                return (_ROUTINES_015,)
            if revision in {"016_horizon_prereq_corr", "020_horizon_prereq_corr_live"}:
                return (_ROUTINES_016,)
            if revision == "015_recover_executor_contract_failure":
                return (_ROUTINES_015_RECOVER,)
            if revision == "016_recover_exhausted_executor_contract_once":
                return (_ROUTINES_016_RECOVER,)
            if revision == "017_parent_rollback_routine":
                return (_ROUTINES_017,)
            if revision == "014_requeue_blocked_parent_task":
                return (_ROUTINES_013,)
...
def _assert_sql_allowlist_projection(revision: str, catalog: dict[str, tuple[str, ...]]) -> None:
    ...
    if revision == "013_cleanup_expired_attempt":
        ...
        return
    if revision == "014_horizon_project_ledger":
        if catalog["public_routines"] != _ROUTINES_014:
            raise RuntimeError(
                "014_horizon_project_ledger catalog routines differ from the reviewed projection"
            )
        return
    if revision == "015_subworkflow_handoff":
        if catalog["public_routines"] != _ROUTINES_015:
            raise RuntimeError(
                "015_subworkflow_handoff catalog routines differ from the reviewed projection"
            )
        return
    if revision == "016_horizon_prereq_corr":
        if catalog["public_routines"] != _ROUTINES_016:
            raise RuntimeError(
                "016_horizon_prereq_corr catalog routines differ from the reviewed projection"
            )
        return
    if revision == "018_horizon_project_ledger_live":
        if catalog["public_routines"] != _ROUTINES_014:
            raise RuntimeError(
                "018_horizon_project_ledger_live catalog routines differ from the reviewed projection"
            )
        return
    if revision == "019_subworkflow_handoff_live":
        if catalog["public_routines"] != _ROUTINES_015:
            raise RuntimeError(
                "019_subworkflow_handoff_live catalog routines differ from the reviewed projection"
            )
        return
    if revision == "020_horizon_prereq_corr_live":
        if catalog["public_routines"] != _ROUTINES_016:
            raise RuntimeError(
                "020_horizon_prereq_corr_live catalog routines differ from the reviewed projection"
            )
        return
    projections = {
...
EXPECTED_MIGRATION_SQL_PROJECTION_DIGEST: Final[str] = (
    "7d3a67b140f87566548031edbb2058135b7b74bd6a77acaccc367efb1a898d6c"
)
