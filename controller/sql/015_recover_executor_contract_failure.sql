-- Fail closed unless 014 requeue and the 014 mutation-scope body are present.
-- Do not apply to top_delivery_control_p1 from this envelope.
DO $td015pred$
DECLARE
  def text;
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'longspan_requeue_blocked_parent_task'
  ) THEN
    RAISE EXCEPTION '015 refuses to apply: 014 requeue predecessor is missing';
  END IF;
  BEGIN
    def := pg_get_functiondef('public.require_longspan_mutation_scope()'::regprocedure);
  EXCEPTION WHEN undefined_function THEN
    RAISE EXCEPTION '015 refuses to apply: 014 mutation-scope predecessor is missing';
  END;
  IF position('requeue_blocked_parent_task' in def) = 0 THEN
    RAISE EXCEPTION '015 refuses unknown or non-014 mutation-scope predecessor body';
  END IF;
END
$td015pred$;

-- Candidate 015 workflow function. Do not apply to top_delivery_control_p1
-- from this envelope. Live remains at 014_requeue_blocked_parent_task.
-- Recovers a terminal non-live Executor contract failure within retry budget.
CREATE OR REPLACE FUNCTION public.longspan_recover_executor_contract_failure(
    p_run_id TEXT,
    p_task_id TEXT,
    p_attempt_id TEXT,
    ...
    p_expire_running_bridge BOOLEAN DEFAULT false
) RETURNS JSONB AS $$
DECLARE
    ...
    v_retry_key TEXT;
    v_outcome TEXT;
    v_replay_recovery BOOLEAN := false;
    v_retry_row public.retry_queue%ROWTYPE;
    v_inserted INTEGER;
    v_bridge_running BOOLEAN := false;
BEGIN
    IF session_user <> 'top_delivery_workflow' THEN
        RAISE EXCEPTION 'executor-contract recovery requires the workflow principal';
    END IF;
    IF p_max_retries IS DISTINCT FROM 5 THEN
        RAISE EXCEPTION 'executor-contract recovery retry ceiling is fixed at 5';
    END IF;
    IF p_owner IS NULL OR btrim(p_owner) = '' THEN
        RAISE EXCEPTION 'executor-contract recovery requires the pinned owner';
    END IF;
    IF p_expected_attempt IS NULL OR p_expected_attempt < 1 THEN
        RAISE EXCEPTION 'executor-contract recovery requires the expected attempt number';
    END IF;
    IF p_fence_token IS NULL OR p_fence_token < 1 THEN
        RAISE EXCEPTION 'executor-contract recovery requires the fence token';
    END IF;
    IF p_failure_class IS DISTINCT FROM 'malformed_structured_output' THEN
        RAISE EXCEPTION 'executor-contract recovery accepts only malformed_structured_output';
    END IF;
    ...
    IF NOT FOUND
       ...
       OR control_row.lease_expires_at <= clock_timestamp() THEN
        RAISE EXCEPTION 'executor-contract recovery is not bound to the live controller lease';
    END IF;
    ...
    FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'executor-contract recovery target task is missing';
    END IF;
    IF parent_row.attempt IS DISTINCT FROM p_expected_attempt THEN
        IF parent_row.state = 'queued'
           AND parent_row.active_attempt_id IS NULL
           AND parent_row.attempt = p_expected_attempt + 1 THEN
            v_replay_recovery := true;
        ELSE
            RAISE EXCEPTION 'executor-contract recovery expected attempt mismatch';
        END IF;
    END IF;
    ...
    FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'executor-contract recovery attempt is missing';
    END IF;
    IF attempt_row.fence_token IS DISTINCT FROM p_fence_token THEN
        RAISE EXCEPTION 'executor-contract recovery fence token mismatch';
    END IF;
    IF attempt_row.status = 'running' OR attempt_row.lease_expires_at > clock_timestamp() THEN
        IF p_expire_running_bridge
           AND attempt_row.status = 'running'
           AND attempt_row.fence_token = p_fence_token
           AND attempt_row.controller_epoch = p_controller_epoch
           AND parent_row.state = 'leased'
           AND parent_row.active_attempt_id = p_attempt_id THEN
            v_bridge_running := true;
        ELSE
            RAISE EXCEPTION 'executor-contract recovery refuses a live attempt lease';
        END IF;
    END IF;
    IF NOT v_bridge_running AND attempt_row.status NOT IN ('failed', 'stale') THEN
        RAISE EXCEPTION 'executor-contract recovery requires a terminal failed or stale attempt';
    END IF;
    IF EXISTS (
           SELECT 1
        ...
        RAISE EXCEPTION 'executor-contract recovery refuses historical attempt binding';
    END IF;
    IF parent_row.state NOT IN ('failed', 'leased', 'queued') THEN
        RAISE EXCEPTION 'executor-contract recovery refuses blocked or foreign parent state';
    END IF;
    IF parent_row.state = 'leased'
       AND parent_row.active_attempt_id IS DISTINCT FROM p_attempt_id THEN
        RAISE EXCEPTION 'executor-contract recovery leased parent attempt mismatch';
    END IF;
    IF parent_row.state = 'failed'
       AND parent_row.active_attempt_id IS NOT NULL
       AND parent_row.active_attempt_id IS DISTINCT FROM p_attempt_id THEN
        RAISE EXCEPTION 'executor-contract recovery failed parent attempt mismatch';
    END IF;
    IF parent_row.state = 'queued' AND parent_row.active_attempt_id IS NULL THEN
        v_retry_key := 'retry:' || p_run_id || ':' || p_task_id || ':attempt:' || parent_row.attempt::TEXT;
        SELECT * INTO v_retry_row
        FROM public.retry_queue
        WHERE retry_key = v_retry_key;
        IF NOT FOUND
           OR v_retry_row.run_id IS DISTINCT FROM p_run_id
           OR v_retry_row.task_id IS DISTINCT FROM p_task_id
           OR v_retry_row.attempt IS DISTINCT FROM parent_row.attempt
           OR v_retry_row.reason IS DISTINCT FROM 'recover_executor_contract_failure'
           OR v_retry_row.state IS DISTINCT FROM 'queued' THEN
            RAISE EXCEPTION 'executor-contract recovery queued parent lacks matching retry record';
        END IF;
        RETURN jsonb_build_object(
            ...
            'replay', v_replay_recovery
        );
    END IF;
    IF parent_row.attempt >= p_max_retries THEN
        v_outcome := 'exhaust';
        IF parent_row.state = 'failed' AND parent_row.active_attempt_id IS NULL THEN
            RETURN jsonb_build_object(
                'recovered', false,
                'reason', 'exhausted_retry_budget',
                'disposition', 'EXHAUSTED_RETRY_BUDGET',
                'task_id', parent_row.task_id,
                'attempt', parent_row.attempt,
                'state', parent_row.state,
                'prior_attempt_id', p_attempt_id
        ...
        END IF;
        next_attempt := parent_row.attempt;
    ELSE
        v_outcome := 'requeue';
    ...
    IF mac_key IS NULL THEN
        RAISE EXCEPTION 'executor-contract recovery MAC material is unavailable';
    END IF;
    scope_payload := jsonb_build_object(
        ...
        'operation', 'recover_executor_contract_failure',
        ...
        'outcome', v_outcome,
        'bridge_expire', v_bridge_running
    );
    ...
    IF v_bridge_running THEN
        ...
        WHERE attempt_id = p_attempt_id
          ...
          AND status = 'running';
        IF NOT FOUND THEN
            RAISE EXCEPTION 'executor-contract recovery in-flight bridge lost the running row';
        END IF;
        SELECT * INTO attempt_row
        FROM public.task_attempts
        WHERE attempt_id = p_attempt_id AND run_id = p_run_id AND task_id = p_task_id
    ...
    IF v_outcome = 'exhaust' THEN
        ...
        WHERE task_id = p_task_id
          ...
          AND (
              ...
              OR (state = 'failed' AND active_attempt_id IS NULL)
          );
        IF NOT FOUND THEN
            RAISE EXCEPTION 'executor-contract recovery lost the exhausted-task fence';
        END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'executor-contract recovery lost the task fence';
    END IF;
    ...
    VALUES (
        ...
        'recover_executor_contract_failure', 'queued'
    )
    ...
    IF v_inserted = 0 THEN
        ...
        IF NOT FOUND
           OR v_retry_row.run_id IS DISTINCT FROM p_run_id
           OR v_retry_row.task_id IS DISTINCT FROM p_task_id
           OR v_retry_row.attempt IS DISTINCT FROM next_attempt
           OR v_retry_row.reason IS DISTINCT FROM 'recover_executor_contract_failure'
           OR v_retry_row.state IS DISTINCT FROM 'queued' THEN
            RAISE EXCEPTION 'executor-contract recovery retry row conflict';
        END IF;
    END IF;
    PERFORM set_config('top_delivery.mutation_scope_payload', '', true);
    PERFORM set_config('top_delivery.mutation_scope_signature', '', true);
...
REVOKE ALL ON FUNCTION public.longspan_recover_executor_contract_failure(TEXT, TEXT, TEXT, INTEGER, TEXT, INTEGER, TEXT, INTEGER, BIGINT, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.longspan_recover_executor_contract_failure(TEXT, TEXT, TEXT, INTEGER, TEXT, INTEGER, TEXT, INTEGER, BIGINT, BOOLEAN)
